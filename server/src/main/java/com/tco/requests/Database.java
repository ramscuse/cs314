...
class Database {
        ...
        static Places places(String match, Integer limit) throws Exception {
			 ...
			 try (
			 	...
			 	//return convertQueryResultsToPlaces(results, "id,name,municipality,iso_region,iso_country,continent,latitude,longitude,altitude");
				return convertQueryResultsToPlaces(results, "world.name,world.municipality,region.name,country.name,continent.name,latitude,longitude,altitude");
			 } catch (Exception e) {
			 	throw e;
			 }
		}
